...
#       source: publib.boulder.ibm.com/infocenter/iseries/v5r4/topic/rzaha/javap
.htm
#
#   3. If the class contains the main function run the java command to run it an
d exit
...
for classfile in *.class; do
    ...
    #Execute fgrep with -q option to not display anything on stdout when the mat
ch is found
    if javap -public $classname | fgrep -q 'public static void main(java.lang.St
ring[])'; then
        java $classname "$@"
        exit 0;
    fi
